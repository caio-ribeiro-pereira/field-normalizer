...
/**
 * Validates if string is compose of letters
 *
 * @param {String} str
 * @return {Boolean} true is string is compose of letter
 * @private
 */
...
const transform = (object, functionName) => {
  ...
  return keys.reduce((normalized, key) => {
    ...
    const hasChineseKey=hasChinese(key);
    const transformedKey = native ? key[functionName]() : !hasChineseKey?changeCase[functionName](key):key;
